...
using MyDashWallet.Services.Tests.Mocks;
using NUnit.Framework;

namespace MyDashWallet.Services.Tests
{
	public class DashNodeTests
	{
		...
		{
			Assert.That(node.CalculateTxFee(1, false, false), Is.EqualTo(0.00000226m));
			Assert.That(node.CalculateTxFee(2, false, false), Is.EqualTo(0.00000374m));
			Assert.That(node.CalculateTxFee(3, false, false), Is.EqualTo(0.00000522m));
			Assert.That(node.CalculateTxFee(7, false, false), Is.EqualTo(0.00001114m));
		}

		[Test]
		public void CheckInstantSendFee()
		{
			Assert.That(node.CalculateTxFee(1, true, false), Is.EqualTo(0.0001m));
			var inputs = new[] { 0.1m, 0.0001m, 0.5m };
			Assert.That(node.CalculateTxFee(inputs.Length, true, false), Is.EqualTo(0.0003m));
		}
		...
		public void CheckPrivateSendFee()
		{
			...
			Assert.That(node.CalculateTxFee(inputs.Length, false, false), Is.EqualTo(0.00001262m));
			Assert.That(node.CalculateTxFee(inputs.Length, false, true),  Is.EqualTo(0.00019262m));
			Assert.That(node.CalculateTxFee(inputs.Length, true, true),   Is.EqualTo(0.00098m));
		}
		...
		public void GenerateRawTx()
		{
			// This will only work in testnet (debug mode)
			var sendAmount = 1m;
			var txFee = node.CalculateTxFee(1, false, false);
			var inputAmount = 10m;
		...
		public void BuildPrivateSendTransaction()
		{
			...
			decimal inputTxFee = node.CalculateTxFee(1, false, true);
			...
			decimal redirectedTxFee = node.CalculateTxFee(1, false, false);
		...
		public void SendSignedTransactionWithInstantSend()
		{
			...
			var sendAmount = 0.1m;
			var txFee = node.CalculateTxFee(1, true, false);
		...
		public void GenerateRealTransaction()
		{
			...
			var txFee = node.CalculateTxFee(2, false, false);
