...
namespace MyDashWallet.Node
{
	...
	public class TestnetDashNode : DashNode
	{
		...
		public override decimal GetUserAddressReceived(string userAddress)
		{
			try
			{
				var response =
		...
		public override string GenerateNewAddress(string userLabel, bool forPrivateSendTx)
			=> service.GetNewAddress(userLabel);
		...
		{
			//for privatekey signing we need to use the third parameter!
